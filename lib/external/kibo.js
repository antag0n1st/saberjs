var Kibo = function (element, context) {
...
Kibo.WILDCARD_TYPES = ['arrow', 'number', 'letter', 'f', 'symbols', 'decimals'];
...
Kibo.WILDCARDS = {
    ...
    symbols: [59, 61, 173, 186, 187, 188, 189, 190, 191, 192, 106, 107, 108, 109, 110, 111, 219, 220, 221, 222],
    decimals: [110, 190]
};
...
Kibo.prototype.handler = function (upOrDown) {
    ...
    return function (e) {
        ...
        if (e.keyIdentifier) {
            e.keyCode = parseInt(e.keyIdentifier, 16);
        ...
        for (i = 0; i < registeredKeys.any.length; i++) {
            var func = registeredKeys.any[i];
            var result = func.call(that.context, e);
            ;
        ...
        for (i = 0; i < Kibo.WILDCARD_TYPES.length; i++)
            if (Kibo.arrayIncludes(Kibo.WILDCARDS[Kibo.WILDCARD_TYPES[i]], that.lastKeyCode)) {
                for (j = 0; j < registeredKeys['any ' + Kibo.WILDCARD_TYPES[i]].length; j++) {
                    var func = registeredKeys['any ' + Kibo.WILDCARD_TYPES[i]][j];
        ...
        if (registeredKeys[lastModifiersAndKey])
            for (i = 0; i < registeredKeys[lastModifiersAndKey].length; i++) {
                var func = registeredKeys[lastModifiersAndKey][i];
                if ((func.call(that.context, e) === false) && e.preventDefault) {
                    e.preventDefault();
...
Kibo.prototype.unregisterAll = function () {

    this.unregisterByKey('keysUp');
    this.unregisterByKey('keysDown');

};

Kibo.prototype.unregisterByKey = function (key) {
