(function (window, undefined) {
    ...
    Keyboard.prototype.initialize = function (delegate, orientation, type) {
        ...
        this.fontSize = null;
        this.keySpacingX = Config.keyboardSpacingX || 5;
        this.keySpacingY = Config.keyboardSpacingY || 10;
        ...
        this.preview = new Sprite('keyboard_preview_key');
        this.preview.anchor.set(0.5, 1);
        this.preview.tint = 0x555555;

        this.preview.foreground = new Sprite('keyboard_preview_key');
        this.preview.foreground.tint = this.colors.preview.background;
        this.preview.foreground.anchor.set(0.5, 1);
        this.preview.foreground.position.set(-1, -1);
        this.preview.addChild(this.preview.foreground);

        this.preview.label = new Label();
        this.preview.label.centered();
        this.preview.addChild(this.preview.label);

        this.preview.label.style.fontFamily = Config.keyboardFont || 'Arial';
        this.preview.label.y = -140;
        this.preview.label.style.fill = this.colors.preview.text;
    ...
    Keyboard.prototype.recalculateSize = function () {
        ...
        if (app.width < app.height) {
            ...
            // horizontal
            this.cellWidth = app.width / 10;
            this.cellHeight = app.height / 8;
            fontFactor = 1.3;
        }

        this.fontSize = Config.keyboardFontSize || (this.cellHeight * 0.4 * fontFactor);

        this.preview.label.style.fontSize = Math.round(this.fontSize * (Config.keyboardPreviewScale || 4));
        this.preview.scale.set(Config.keyboardPreviewBubbleScale || 0.5);
    ...
    Keyboard.prototype.backgroundSetup = function () {
        ...
        this.backgorund.beginFill(this.colors.layoutBackground, 1);
    ...
    Keyboard.prototype.layoutSetup = function () {
        ...
        var symbolsButton = this.createButton(this.isSymbol ? 'abc' : '123', symbolsWidth, cellHeight, this.keySpacingX, this.keySpacingY, this.colors.specialKeys);
        symbolsButton.foreground.tint = this.colors.specialKeys.background;
        symbolsButton.symbol = 'keyboard_symbols';
        symbolsButton.anchor.set(0.5, 0.5);
        symbolsButton.isSpecial = true;

        var space = this.createButton(null, spaceWidth, cellHeight, this.keySpacingX, this.keySpacingY, this.colors.specialKeys);
        space.foreground.tint = this.colors.specialKeys.background;
        space.symbol = 'space';
        ...
        var backspace = this.createButton(backIcon, cellWidth * 1.3, cellHeight, this.keySpacingX, this.keySpacingY, this.colors.specialKeys);
        backspace.foreground.tint = this.colors.specialKeys.background;
        ...
        var doneButton = this.createButton(this.hasNext ? this.nextText : this.doneText, doneWidth, cellHeight, this.keySpacingX, this.keySpacingY, this.colors.done);
        doneButton.foreground.tint = this.colors.done.background;
    ...
    Keyboard.prototype.createButton = function (symbol, width, height, spacingX, spacingY, colors) {
        ...
        button.label.style.fill = this.colors.keys.text;
        button.label.style.fontSize = this.fontSize;
        button.label.style.fontFamily = Config.keyboardFont || 'Arial';
        button.label.style.fontWeight = Config.keyboardFontWeight || 700;
        button.priority = 100;
    ...
    Keyboard.prototype.keyMouseDown = function (event, object) {
        ...
        if (!object.isSpecial) {
            ...
            this.preview.position.set(x, p.y - this.previewKeyOffsetY);
        ...
        if (Config.keyboardVibrate && window.navigator && window.navigator.vibrate) {
